...
// importation du contactForm.js pour récupérer le photographerName
import { changeContactFormName } from "../utils/contactForm.js";
...
async function init() {
  ...
  changeContactFormName(photographer.name);
...
async function displayData(photographer) {
  ...
  function carousel() {
    ...
    let mediaPhotographer = document.querySelectorAll(".media-photographer");
    ...
    mediaPhotographer.forEach((image) => {
      image.addEventListener("click", (event) => {
        if (!event.target.classList.contains("fa-heart")) {
          ...
          if (imageLightBox.querySelector("img")) {
            const imagealt = imageLightBox
              .querySelector("img")
              .getAttribute("alt");
            imageLightBox
              .querySelector("img")
              .setAttribute("alt", imagealt.split(", closeup view")[0]);
    ...
    flecheG.addEventListener("click", (event) => {
    ...
    flecheD.addEventListener("click", (event) => {
      event.stopImmediatePropagation();
      let numberImage = Number(
        container.querySelector(".media-photographer").getAttribute("data-id")
      );
  ...
  // traitement de dans la bar de stat
